...
class Linear:
    ...
    def __init__(self, in_layer, num_out_features):
        ...
        self.out_dims =  (self.num_data,num_out_features)
        # TODO: Declare the weight matrix. Be careful how you initialize the matrix.
        self.W = np.random.randn(self.num_in_features, num_out_features) / np.sqrt(self.num_in_features) ##################
    def forward(self):
        ...
        #self.out_array = np.dot(self.in_layer,self.W.T)
        return self.out_array
    def backward(self, dwnstrm):
        
        input_grad= dwnstrm @ self.W.T

        dwnstrm=dwnstrm.reshape(dwnstrm.shape[0],1,dwnstrm.shape[1])
        augmented_input_array= self.in_array.reshape(self.in_array.shape[0],1,self.in_array.shape[1])  
        
        G=[]

        for i,backprop in enumerate(dwnstrm):
            G.append(augmented_input_array[i].T @ backprop)
...
class Relu:
    ...
    def forward(self):
        ...
        self.out_array = np.maximum(0,self.in_array)

        return self.out_array
    
    def backward(self, dwnstrm):
        # TODO: Compute grad of output with respect to inputs, and hand this gradient backward to the layer behind
        backprop = np.zeros_like(self.in_array)
        backprop[self.in_array > 0 ] = 1
        # hand this gradient backward to the layer behind
        self.in_layer.backward(np.multiply(dwnstrm, backprop))
        pass
    pass
...
class SquareLoss:
    ...
    def forward(self):
        ...
        self.out_array =  np.sum((self.in_array - self.labels)**2)/(2*self.num_data)
        return self.out_array
    def backward(self):
        """Gradient is (1/M) (X-Y), where N is the number of training samples"""
        # TODO: Compute grad of output with respect to inputs, and hand this gradient backward to the layer behind
        self.pass_back = (self.in_array-self.labels) #/self.num_data - calculating mean at the update step
        # hand this gradient backward to the layer behind
...
class Sigmoid:
    ...
    def backward(self, dwmstrm):
        # TODO: Compute grad of output with respect to inputs, and hand this gradient backward to the layer behind. Be careful! Don't exponentiate an arbitrary positive number as it may overflow. 
        backprop = np.zeros_like(dwmstrm)
        for i in range(dwmstrm.shape[0]):
            if (self.in_array[i][0]>0):
                backprop[i][0]= np.exp(-self.in_array[i][0])/((np.exp(-self.in_array[i][0])+1)**2)
            else:
                backprop[i][0]= np.exp(self.in_array[i][0])/((np.exp(-self.in_array[i][0])+1)**2)
            
        self.in_layer.backward(np.multiply(backprop, dwmstrm))
