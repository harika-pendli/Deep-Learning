...
class Network(layers.BaseNetwork):
    ...
    def __init__(self, data_layer,parameters=None):
        ...
        for i in range(self.hidden_layers+1):
             self.MY_MODULE_LIST.append(layers.Linear(self.MY_MODULE_LIST[-1],self.hidden_units))
...
class Trainer:
    ...
    def setup(self, training_data):
        ...
        hidden_layers = 2
        hidden_units = 15
        parameters={"hidden_units": hidden_units,"hidden_layers":hidden_layers}  ######################change as needed
        self.network = Network(self.data_layer,parameters=parameters)
        #TODO: use the appropriate loss function here
        self.loss_layer = layers.SquareLoss(self.network.get_output_layer(),y) 
        #TODO: construct the optimizer class here. You can retrieve all modules with parameters (thus need to be optimized be the optimizer) by "network.get_modules_with_parameters()"
        self.optim = layers.SGDSolver(0.07, self.network.get_modules_with_parameters())   ###############change as needed
...
def main(test=False):
    ...
    if not test:
        ...
        dataset= data_generators.data_3a()
        ...
        print("Module list:",trainer.network.get_modules_with_parameters())
